...
describe('respondToMention', () => {
  test('a fix-pr mention posts its summary after pushing to the PR head', async () => {
    ...
    let started: AgentStartOptions | undefined
    const kind = await respondToMention({
      ...
      exec,
      makeHarnessFn: () => {
        const base = summaryHarness(
          'Fixed the parser edge case.',
          join(tmpdir(), 'amagi-fix-pr-7-1.md'),
        )
        return {
          ...base,
          start: (opts: AgentStartOptions) => {
    ...
    expect(inputs[0]).toContain('[am-1] am-1')
    expect(inputs[0]).toContain('Fixed the parser edge case.')
    expect(inputs[0]?.trim().split('\n').at(-1)).toMatch(/^Generated by amagi/)
    expect(lintCommitMessage(inputs[0] ?? '')).toEqual([])
    expect(started?.env).toEqual({ AMAGI_WORKTREE: '/wt/amagi-pr-7', AMAGI_REPO_ROOT: '/repo' })
    expect(bypassed).toEqual([['bbb reset: unexpected']])
  })

  test('a fix-pr mention with no tracker keeps the legacy commit message', async () => {
    const { exec, inputs } = fake((c) =>
      c[1] === 'status' ? { exitCode: 0, stdout: ' M src/fix.ts\n', stderr: '' } : undefined,
    )
    await respondToMention({
      root: '/repo',
      repoName: 'amagi',
      pr: pr(),
      mention: { id: 'legacy', user: 'bob', body: 'please fix this' },
      config: config(),
      driver: new FakeDriver(),
      exec,
      makeHarnessFn: () =>
        summaryHarness('Fixed the parser edge case.', join(tmpdir(), 'amagi-fix-pr-7-legacy.md')),
    })

    expect(inputs).toEqual([
      'Respond to review feedback on PR #7\n\nPR: https://github.com/owner/repo/pull/7',
    ])
  })
