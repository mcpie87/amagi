...
describe('formatPrBody', () => {
  ...
  test('keeps the machine-readable task id before the footer', () => {
    const body = formatPrBody(TASK, [], {
      harness: 'opencode',
      model: 'gemini-2.5-pro',
      effort: 'high',
    })
    expect(body).toContain('<!-- amagi-task: am-1 -->')
...
describe('taskIdFromPrBody', () => {
  test('reads the hidden task id comment back off a PR body', () => {
    const body = formatPrBody(TASK, [])
