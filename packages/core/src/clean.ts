...
export async function cleanTerminalWorktrees(
  ...
  for (const task of store.tasks({ states: TERMINAL_STATES, limit: 10_000 })) {
    ...
    if (existsSync(plan.path)) {
      const r = await run(['git', 'worktree', 'remove', '--force', plan.path], {
        cwd: opts.repoRoot,
      })
      if (r.exitCode !== 0) throw new Error(`git worktree remove failed: ${r.stderr.trim()}`)
    }
    if (plan.branch !== null && (await branchExists(run, opts.repoRoot, plan.branch))) {
      const r = await run(['git', 'branch', '-D', plan.branch], { cwd: opts.repoRoot })
      if (r.exitCode !== 0) throw new Error(`git branch -D failed: ${r.stderr.trim()}`)
    }

    store.append(task.id, { type: 'worktree.removed', path: task.worktree })
  }
